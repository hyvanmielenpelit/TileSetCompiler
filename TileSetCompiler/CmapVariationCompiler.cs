    ...
    class CmapVariationCompiler : DungeonTileCompiler
    {
        ...
        public override void CompileOne(string[] splitLine)
        {
            ...
            if (lastDash > 0 && lastDash < name.Length - 1)
            {
                ...
                if(isNumeral)
                {
                    nameWithoutIndex = name.Substring(0, lastDash);
                }
            }
            ...
            else if (_autogeneratedDarkTiles.ContainsKey(nameWithoutIndex))
            {
                ...
                using (var image = DarknessCreator.CreateDarkBitmapFromFile(sourceFile, name, info.Opacity, out isTileMissing))
                {
                    if (!isTileMissing)
                    {
                        WriteCmapTileNameAutogenerationSuccess(sourceFilePath, relativePath, "cmap", null);
                    }
                    else
                    {
                        ...
                        WriteCmapTileNameAutogenerationError(sourceFilePath, relativePath, "cmap", null);
                    }
            ...
            else
            {
                ...
                else
                {
                    ...
                    isTileMissing = true;
                    WriteCmapTileNameErrorFileNotFound(relativePath, "Creating Missing Cmap Variation tile.", null);
                }
                ...
                {
                    using (var image = MissingCmapVariationTileCreator.CreateTile(_missingCmapType, null, name))
                    {
                        DrawImageToTileSet(image);
                    }
                }
